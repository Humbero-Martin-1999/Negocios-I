...
export class ProductService {
  ...
  private loadProductsFromStorage() {
    const productsString = localStorage.getItem(this.localStorageKey);
    if (productsString) {
      // Si hay productos en localStorage, los cargamos
      this.products = JSON.parse(productsString);
    } else {
      // Si no, cargamos el arreglo inicial y lo guardamos
      this.products = [
        // Leggings (IDs 1–14)
        ...
        // Suplementos Crossfit (IDs 82–85)
        ...
        // Suplementos Yoga-Bienestar (IDs 86–89)
